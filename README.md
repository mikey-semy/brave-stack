...
Сборка self-hosted сервисов для одного небольшого сервера.
...
Сборка крутится на одном VPS (1 vCPU / 2 ГБ, Debian 13): семь контейнеров,
standalone-compose, **без Dokploy** — файлы `docker-compose.dokploy.yml` лежат
про запас и ни разу не применялись. Ежедневный off-site бэкап с GPG.
